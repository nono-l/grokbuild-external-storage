...
PHP;
                        ...
                        $config_content = <<<PHP
<?php
// Non-secret. Written by install.php — safe-ish to leave on disk.
// Secrets are in api/secrets.local.php (not in git / not in source).
...
PHP;
                        ...
                        $secrets_content = <<<PHP
<?php
// GENERATED by install.php — do not commit. Rotate via setup.php.
...
PHP;
                        ...
                        $secrets_path = __DIR__ . '/api/secrets.local.php';
                        $wrote_cfg = file_put_contents($config_path, $config_content);
                        $wrote_sec = file_put_contents($secrets_path, $secrets_content);
                        if ($wrote_cfg && $wrote_sec) {
                            @chmod($secrets_path, 0600);
                            $success = true;
                            $apiKeyHint = $apiKey;
                            @rename(__FILE__, __DIR__ . '/install.php.txt');
                        } else {
                            $error = 'config / secrets.local.php の書き込みに失敗しました。権限を確認してください。';
                        }
                    } else {
